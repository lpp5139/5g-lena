 ...
 * different CCs.
 *
 * We prov"cttc-nr-cc-bwp-demoide a number of simulation parameters that can be configured in the
...
main (int argc, char *argv[])
{
  ...
  double bandwidthBand = 6e9;

  bool contiguousCc = true;
  uint16_t numerology = 0;                      //numerology for contiguous case
  uint16_t numerology2 = 2;
  //non-contiguous case
  ...
  uint32_t udpPacketSizeBe = 5972; //2472; //1472; //972
  uint32_t lambdaUll = 40;
  uint32_t lambdaBe = 1000;
  srand(time(NULL));

  //double random_number = ((double)rand()) / INT_MAX;
  ...
  double simTime = 200; // seconds
  double udpAppStartTime = 0.0; //seconds
  ...
  if (contiguousCc == true)
    {
      ...
      /*nrHelper->GetGnbPhy (enbNetDev.Get (0), 1)->SetAttribute ("Numerology", UintegerValue (numerology2));
...
*/
 ...
 // p2ph.DisableFlowControl();
  //p2ph.SetQueue ("ns3::DropTailQueue", "MaxSize", StringValue("10000p"));
  //p2ph.EnableFlowControl();
          ...
          if (!disableDl)
            {
              ...
              Ptr<Socket> ns3TcpSocket = Socket::CreateSocket (remoteHost, UdpSocketFactory::GetTypeId ());
                app->Setup (ns3TcpSocket, sinkAddress, 1500, 100000, DataRate ("1Mbps"));
              ...
              dlClient.SetAttribute ("Interval", TimeValue (Seconds (1.0 / lambdaUll)));
	            //dlClient.SetAttribute("Interval", TimeValue (Seconds (lambdaUl->GetValue()))); 
  ...
  app->SetStartTime (Seconds (udpAppStartTime));
  ...
  p2ph.EnableAsciiAll (ascii.CreateFileStream ("lena-exp.tr"));
  p2ph.EnablePcapAll ("lena-exp", true);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      outFile << "  Total time: StartTime - Last byte received " << (i->second.timeLastRxPacket.GetSeconds() - udpAppStartTime) << "s\n";
     // outFile << "Random number" << random_number << "s\n";
	  ...
	  outFile << "Difference between first and last byte recevied " << rxDuration <<"seconds \n";
          outFile << "  Throughput: " << i->second.rxBytes * 8.0 / rxDuration / 1000 / 1000  << " Mbps\n";
          outFile << "  Mean delay:  " << 1000 * i->second.delaySum.GetSeconds () / i->second.rxPackets << " ms\n";
          //outFile << "  Mean upt:  " << i->second.uptSum / i->second.rxPackets / 1000/1000 << " Mbps \n";
          outFile << "  Mean jitter:  " << 1000 * i->second.jitterSum.GetSeconds () / i->second.rxPackets  << " ms\n";
	  outFile << " Total Delay: " << i->second.delaySum.GetSeconds() << "s\n";
        }
      else
        {
